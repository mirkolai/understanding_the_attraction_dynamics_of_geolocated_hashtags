...
for data in db.astenagusia.find():
    if data['location']!=None and 'latitude' in data['location'] and 'longitude' in data['location']:
        ...
        posts.append({  'location': [round(float(data['location']['latitude'])+random.uniform(0.0001, 0.00001),5),
...
for day in (mindate + datetime.timedelta(days=n) for n in range((maxdate-mindate).days)):
    ...
    for hour in (day + datetime.timedelta(hours=m) for m in range(24)):

        scan = ddbscan.DDBSCAN(eps=0.1, min_pts=1) #eps radius to look for neighbours

        for post in posts:

            if post['created_time']>= hour and post['created_time'] < hour + datetime.timedelta(hours=1) :
        ...
        results_hours.append({
                                ...
                                'date': hour.strftime('%Y-%m-%d %H:%M:%S'),
                                'clusters': clusters
                            })

with open('cluster_analysis_hours.json', 'w') as outfile:
